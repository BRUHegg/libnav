...
double strtod(const std::string& s) noexcept {
  ...
  while (s[d_idx] != '.' && d_idx < s.size()) d_idx++;
...
std::string deg_to_str(double abs_deg, char deg_sbl) noexcept {
  std::string s;
  std::vector<int> repr;
...
std::string lat_to_str(double lat_deg, char deg_sbl) noexcept {
  std::string s;
...
double str_to_lat(const std::string& s) noexcept {
  if (int(s.length()) == N_LAT_STR_LENGTH) {
    ...
    double lat = digit_pairs[0] + digit_pairs[1] / 60 + digit_pairs[2] / 3600 +
                 digit_pairs[3] / 360000;
...
std::string lon_to_str(double lon_deg, char deg_sbl) noexcept {
  std::string s;
...
double str_to_lon(const std::string& s) noexcept {
  if (int(s.length()) == N_LON_STR_LENGTH) {
    ...
    double lon = digit_pairs[0] + digit_pairs[1] / 60 + digit_pairs[2] / 3600 +
                 digit_pairs[3] / 360000;
...
std::vector<std::string> str_split(const std::string& in, char sep,
  ...
  if (!n_split) {
    std::getline(s, tmp);
    if (tmp.length()) out.push_back(strip(tmp, '\r'));
  }
...
std::string get_rnw_id(const std::string& id, bool ignore_all) noexcept {
